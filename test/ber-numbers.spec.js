...
describe('Test length function', () => {
  test('Should return 1 for values less then 128', () => {
    ['1', '10', '40', '100', '127'].forEach(val => expect(ber.length(val)).toBe(1));
  });
...
describe('Test encode function', () => {
  [
    ...
    test(`Should return ${testValue.encoded} for ${testValue.decoded}, base: ${testValue.base}`, () => {
      expect(ber.encode(testValue.decoded, testValue.base)).toBe(testValue.encoded);
    });
  });
